...
function getGenAI(clientKey?: string): GoogleGenAI {
  ...
  if (finalKey && finalKey.length > 25 && !finalKey.startsWith("AIzaSy")) {
  ...
  const rawKey = finalKey || dynamicServerKey || envKey || MASTER_GEMINI_KEY;
  const key = rawKey ? rawKey.trim() : "";
  
  if (key) {
    console.log(`[getGenAI Selection]: Selected key length is ${key.length}, prefix is ${key.substring(0, 6)}`);
  }
...
async function generateContentWithRetry(params: GenerateContentWithRetryParams, clientKey?: string, maxRetries = 4): Promise<any> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const is403 = errorStr.includes("denied") || errorStr.includes("permission_denied") || errorStr.includes("403");
      ...
      if (isInvalidKey) {
        if (!clientKey) {
          throw new Error(
            "KUNCI API INTERNAL TIDAK VALID:\n\n" +
            "Kunci API Gemini bawaan di server terdeteksi tidak valid.\n\n" +
            "💡 CARA MEMPERBAIKI:\n" +
            "Harap masukkan Kunci API Gemini mandiri milik Anda ke dalam panel 'MASUKKAN GEMINI_API_KEY' di bagian atas halaman aplikasi. " +
            "Anda dapat membuat Kunci API gratis di website ai.google.dev (menggunakan akun Google Anda), lalu tempelkan ke kolom yang disediakan."
          );
        } else {
          throw new Error(
            "KUNCI API MANDIRI TIDAK VALID:\n\n" +
            "Kunci API Gemini yang Anda masukkan di bagian atas aplikasi terdeteksi tidak valid atau salah ketik.\n\n" +
            "💡 CARA MEMPERBAIKI:\n" +
            "Periksa kembali kunci yang Anda masukkan. Pastikan Anda menyalin seluruh karakter kunci (biasanya dimulai dengan 'AIzaSy') tanpa spasi tambahan, lalu simpan kembali."
          );
        }
      }
      ...
      if ((!isTransient && !is403) || attempt === maxRetries) {
        if (is403) {
          if (!clientKey) {
            throw new Error(
              "AKSES API TERBATAS (403/PERMISSION_DENIED):\n\n" +
              "Kunci API Gemini bawaan server saat ini mengalami pembatasan akses atau ditolak oleh sistem Google Cloud/AI Studio.\n\n" +
              "💡 SILAKAN GUNAKAN KUNCI API ANDA SENDIRI:\n" +
              "Anda dapat memperoleh API Key Gemini gratis dalam 1 menit di website ai.google.dev (pastikan untuk login dengan email Google Anda).\n" +
              "Setelah mendapat kunci, harap masukkan/tempel kunci tersebut ke dalam kolom kuning 'MASUKKAN GEMINI_API_KEY' di bagian atas halaman utama aplikasi ini."
            );
          } else {
            throw new Error(
              "KUNCI API TIDAK VALID (403/PERMISSION_DENIED):\n\n" +
              "API Key Gemini mandiri yang Anda masukkan ditolak oleh Google. Harap periksa kembali ejaan kunci Anda atau pastikan kunci tersebut masih aktif."
            );
          }
        }
        throw error;
      ...
      if (fallbackModels.length > 0) {
        const nextFallback = fallbackModels.shift();
        if (nextFallback) {
          console.warn(`[Gemini API] Error on "${currentModel}". Shifting failover target to next fallback model: "${nextFallback}"`);
...
async function startServer() {
  ...
  app.post("/api/save-key", (req, res) => {
    try {
      ...
      if (decodedKey && decodedKey.startsWith("AIzaSy")) {
...
- Provinsi Wilayah Sekolah: ${provinsi || "Nusa Tenggara Timur"}
...
2. SANGAT RINCI & HINDARI SINGKATAN: Dilarang keras menyingkat materi atau menyederhanakan penjelasan dengan kata-kata seperti 'dan seterusnya', 'dll', atau 'dsb'. Seluruh deskripsi kompetensi, lingkup materi, kriteria ketercapaian, dan indikator penilaian harus dijabarkan secara utuh, detail, ilmiah, dan panjang lebar tanpa kompromi!
...
- **Tabel Alur Pembelajaran:** Gambar tabel markdown kolom: [No, Kode TP, Rumusan TP, Alokasi Waktu (JP), Indikator Asesmen/Prasyarat, Semester]. Tuliskan rumusan indikator asesmen secara lengkap, operasional, dan detail tanpa singkatan.
...
- **Tabel Instrumen KKTP:** Anda WAJIB menyajikan baris penilaian detail berbentuk tabel markdown untuk SETIAP SINGLE TP (Tujuan Pembelajaran) yang telah dirumuskan di BAB I dan BAB II tanpa terkecuali! Jika Anda merumuskan 7 TP, maka Anda harus memetakan ke-7 TP tersebut satu per satu ke dalam 7 baris tabel instrumen KKTP ini. DILARANG KERAS hanya menuliskan sebagian atau menggabungkan secara malas. Kolom tabel berkewajiban: [Tujuan Pembelajaran (TP), Kriteria/Indikator, Ketuntasan Baru Berkembang (Skor 0-60%), Layak (Skor 61-70%), Cakap (Skor 71-85%), Mahir (Skor 86-100%)] beserta deskripsi kualitatif performa siswa yang sangat rinci, bermutu ilmiah, dan operasional untuk tiap level kognitif agar guru terbantu mengukur ketuntasan kompetensi secara nyata.
...
- **KEWAJIBAN DETAIL PROTA:** Buat analisis Program Tahunan yang SANGAT DETAIL, UTUH, LENGKAP, DAN SEMPURNA. Tulis dan jabarkan semua rincian kompetensi dan lingkup materi. Di dalam tabel Prota, Anda WAJIB menyajikan baris tabel Prota untuk SELURUH TP yang telah dirumuskan di BAB I dan BAB II tanpa ada satu pun yang tertinggal, diringkas, atau disamarkan! Buat tabel markdown dengan rincian kolom [No, Semester, Kode & Bunyi Tujuan Pembelajaran (TP) Lengkap, Alokasi Waktu (JP)]. Dilarang keras hanya menuliskan kode seperti "TP 1" atau hanya "Memahami listrik" saja; Anda WAJIB menggabungkan kode tersebut dengan rumusan bunyi lengkap TP tersebut secara terperinci (misal: "TP-1.1: Siswa menganalisis konsep gaya listrik dan memanfaatkannya..."). Pastikan jumlah total JP persis selaras dengan analisis mingguan tahunan.
...
- **Tabel Promes Lengkap & Selaras:** Anda WAJIB menyajikan baris tabel Promes untuk SELURUH TP yang dirumuskan di BAB I dan BAB II tanpa ada yang terpotong. Jumlah baris pada gabungan Semester 1 dan Semester 2 harus persis sama dengan seluruh TP yang ada di Bab I/II/IV.
