...
export const LessonPlanner: React.FC = () => {
  ...
  const handleFileRead = (selectedFile: File) => {
    if (selectedFile.type !== "application/pdf" && !selectedFile.name.endsWith(".pdf")) {
      setErrorMsg("Format file tidak didukung. Harap unggah berkas bertipe PDF.");
      return;
    }
    ...
    reader.onload = (e) => {
      try {
        ...
        setFileBase64(b64);
        setFileMime(selectedFile.type || "application/pdf");
        setFileReadingStatus("Dokumen siap diproses.");
      } catch (err) {
        setErrorMsg("Gagal merelasikan berkas.");
        setFileReadingStatus("");
      }
    };
  ...
  const generateLessonPlan = async () => {
    ...
    if (cpInputMode === "file" && !fileBase64) {
      setErrorMsg("Harap unggah naskah dokumen Capaian Pembelajaran (CP) PDF terlebih dahulu.");
      return;
    }
    ...
    try {
      ...
      const response = await fetch("/api/generate-lesson-plan", {
        ...
        body: JSON.stringify({
          ...
          mapel,
          faseKelas,
          cpText: (cpInputMode === "text" || cpInputMode === "bank") ? cpText : "",
          alokasiJp,
          tahunAjaran,
          namaGuru,
          fileBase64: cpInputMode === "file" ? fileBase64 : null,
          mimeType: cpInputMode === "file" ? fileMime : null,
          tempatPenyusunan,
          tanggalPenyusunan,
  ...
  const downloadAsWord = () => {
    ...
    const coverPageHtml = `
      <div style="text-align:center; padding-top:80px; margin-bottom:120px; page-break-after:always;">
        ...
        <div style="margin-top:120px; font-family:'Times New Roman',serif;font-size:9.5pt;color:#333333;border-top:1px solid #000000;padding-top:10px;width:70%;margin-left:auto;margin-right:auto;">
  ...
  const downloadAsPdf = () => {
    try {
      ...
      if (hasMinistry && hasSchool) {
        ...
        coverY += 1;
        coverY = drawCenteredWrappedText("KURIKULUM MERDEKA", 11, "bold", pageWidth - 36, coverY);

        coverY += 8;
        const schLogoSize = 28;
        try {
        ...
        coverY += schLogoSize + 10;

        coverY = drawCenteredWrappedText(namaSekolah.toUpperCase(), 14, "bold", pageWidth - 36, coverY);
        coverY += 10;

        coverY = drawCenteredWrappedText(`MATA PELAJARAN: ${mapel.toUpperCase()}`, 11, "normal", pageWidth - 40, coverY);
        coverY += 1.5;
        coverY = drawCenteredWrappedText(faseKelas.toUpperCase(), 11, "normal", pageWidth - 40, coverY);
        coverY += 1.5;
        coverY = drawCenteredWrappedText(`Penyusun: ${namaGuru}`, 11, "normal", pageWidth - 40, coverY);
        coverY += 1.5;
        coverY = drawCenteredWrappedText(`Tahun Pelajaran: ${tahunAjaran}`, 11, "normal", pageWidth - 40, coverY);
        ...
        // Mode 3: Tanpa logo
        coverY = 48;
        coverY = drawCenteredWrappedText("ANALISIS CP, TP, ATP, KKTP, PROMES DAN PROTA", 15, "bold", pageWidth - 36, coverY);
        coverY += 1;
        coverY = drawCenteredWrappedText("KURIKULUM MERDEKA", 11, "bold", pageWidth - 36, coverY);
      ...
      const triggerNewPage = (targetOrientation?: "portrait" | "landscape") => {
        pageCreatedIndex = 2;
        const targetO = targetOrientation || currentOrientation;
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (!line.startsWith("|")) {
          // Gracefully default back to portrait when landscape tables end!
          if ((currentOrientation as string) === "landscape") {
            switchToOrientation("portrait");
            triggerNewPage("portrait");
          }
        ...
        if (line.startsWith("|") && line.endsWith("|")) {
          ...
          if (!activeTableColWidths) {
            ...
            activeTableOrientation = (totalCols > 7) ? "landscape" : "portrait";
        ...
        if (line.toLowerCase().includes("lembar pengesahan")) {
          // Force layout back to PORTRAIT if landscape was active, then add page
          switchToOrientation("portrait");
      ...
      switchToOrientation("portrait"); // reset orientation just in case
        ...
        <div className="w-full space-y-8 animate-fade-in text-zinc-300">
                  ...
                  {cpInputMode === "file" ? (
                    <div className="space-y-3">
                      <p className="text-[11px] text-zinc-300 leading-relaxed font-sans">
                        💡 <strong>Sistem Otomatis BSKAP 046/2025:</strong> Unggah naskah dokumen PDF CP resmi. Sistem cerdas kami akan menganalisis materi kurikulum secara dinamis didasarkan pada pilihan <strong>Fase/Kelas</strong> dan <strong>Mata Pelajaran</strong> Anda.
                      </p>
                      ...
                      {!cpFile ? (
                        ...
                        >
                          ...
                          <label htmlFor="planner-pdf-uploader" className="cursor-pointer space-y-2 block">
                            ...
                            <div>
                              <p className="text-xs text-zinc-200 font-bold font-sans">Seret berkas PDF CP ke sini atau klik untuk menelusuri</p>
                              <p className="text-[9px] text-zinc-500 font-mono mt-0.5">Format asasi: .pdf (Maks. 30MB)</p>
                            </div>
                          </label>
                        </div>
                      ) : (
          ...
          <div className="flex justify-end pt-5">
            <button
              ...
              disabled={loading || (cpInputMode === "text" && !cpText) || (cpInputMode === "file" && !fileBase64)}
              className={`py-4 px-10 rounded-2xl bg-gradient-to-r from-emerald-500 via-teal-500 to-amber-500 text-black font-extrabold font-sans text-sm flex items-center gap-2.5 shadow-[0_10px_30px_rgba(16,185,129,0.3)] active:scale-95 transition-all cursor-pointer tracking-wide uppercase ${
                loading || (cpInputMode === "text" && !cpText) || (cpInputMode === "file" && !fileBase64) ? "opacity-40 cursor-not-allowed" : "hover:brightness-110 hover:shadow-[0_12px_40px_rgba(245,158,11,0.4)]"
              }`}
            >
              {loading ? (
                <>
